...
export default async function SectionPage({ params }: { params: Promise<{ module: string; section: string }> }) {
  ...
  if (module === 7) {
    const previous = section > 1 ? item.sections[section - 2] : item.title
    const next = section < item.sections.length ? item.sections[section] : courseModules[7]?.title
    return <ReadingFrame module={module} lesson={section} title={title}><ModuleSevenLesson section={section} /><nav aria-label="Lesson navigation" className="mt-12 flex items-center justify-between border-t border-border pt-6 font-mono text-[10px]"><Link href={section > 1 ? `/m/7/s/${section - 1}` : "/m/7"}>← {previous}</Link>{next && <Link className="accent-ink text-right" href={section < item.sections.length ? `/m/7/s/${section + 1}` : "/m/8"}>{next} →</Link>}</nav></ReadingFrame>
  }
