...
const S1_NOTE =
  ...
  "put there. Attention is spread thinner across all of it."
const S5_NOTE =
...
const lessons = [
  ...
  { node: (
    ...
    ), title: "Project: an agent that remembers", minutes: "40", line: "Give your Module 3 agent notes it writes for itself, a summary step when the transcript gets long, and a retrieval step that finds the right note.", figure: "YOUR RUN: window fills → COMPACTION → window drops\n\nnotes.md\n────────────\npreference\ndecision\nFACT RETRIEVED  ← highlighted", caption: "FIGURE 4.7.1 — Your agent, remembering.", library: "Context-bar component fed from traces.", project: true, paragraphs: ["Memory is where agents quietly get good or quietly get worse. You'll be able to see which, turn by turn.", "Start with the notes file alone. Then add compaction and watch the window drop. Add retrieval last, and only when the notes outgrow the window.", "Run ./course init, then tell your agent to start Module 4. When one trace shows the agent writing a note, compaction shrinking the transcript, and retrieval choosing which notes to load, ./course check 4 grades it against your trace, not your output. Leave ./course serve running and the panel above fills with your own run."], key: "Notes give an agent long-term memory; compaction keeps the short-term window sharp. Add retrieval only once the notes outgrow the window.", deeper: "12-factor agents, Factors 3 and 5.", url: "https://github.com/humanlayer/12-factor-agents" },
]
