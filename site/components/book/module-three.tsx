        ...
        <Code lang="python" title="tool definition">{`
{
  ...
  "input_schema": {
    ...
    "properties": {
      ...
      note: "It reads the name, description, and schema the same way it reads everything else.",
    }}
    right={{
...
const lessons = [
  { node: <Fig03 />, title: "A model can't do anything", minutes: "5", line: "A model produces text; it cannot look anything up, run anything, or change anything, unless something around it does those things.", caption: "FIGURE 3.1.1 — The model asks. Your code is what actually runs.", library: "A side-by-side transcript component.", figure: "NO TOOLS                              WITH A TOOL\nuser  What's the weather in Austin?     user  What's the weather in Austin?\nassistant  Austin is 84°F...             calling get_weather(city='Austin')\n                                         Result: 84°F, partly cloudy\n                                         assistant  It's 84°F and partly cloudy in Austin.", paragraphs: ["Module 1 ended with the model confidently inventing the weather. It wasn't being careless. A model never has a way to find out anything: today's date, the contents of a file, the result of a calculation, the state of your calendar.", "A model is text in, text out. That's the whole thing. Everything an AI assistant does beyond producing text (searching, running code, sending an email) is done by ordinary software around the model, on the model's behalf.", "This module is about that software. Once you see where the line is, agents stop being mysterious. The model decides; your code does."], key: "Everything an agent does in the world is done by your code, at the model's request.", question: "Who runs a web search when an agent searches the web?", options: ["The model.", "Your code, because the model asked for it.", "The model provider's servers, automatically."], answers: ["The model can only produce text.", "Yes.", "Only if you set that up, and even then it's still code, not the model."], deeper: "Karpathy's LLM OS sketch — the model as a CPU, tools as peripherals.", url: "https://www.youtube.com/watch?v=zjkBMFhNj_g" },
  { node: fig32, title: "A tool is three things", minutes: "7", line: "A tool is a name and description the model reads, a schema for its arguments, and a function your code runs.", caption: "FIGURE 3.2.1 — The model sees the top two. Your code owns the third.", library: "A three-band tool card.", figure: "DESCRIPTION   get_weather — current weather for a city\nSCHEMA        { city: string }\nFUNCTION      calls a weather API\n\nDESCRIPTION + SCHEMA  →  MODEL\nFUNCTION               →  YOUR CODE", paragraphs: ["To give the model a tool, you don't give it the tool. You give it a description of the tool.", "A tool has three parts. The name and description tell the model what it's for. It reads them in plain words, the way it reads everything else. The schema says what arguments it takes and what type each one is. The function is the code that actually does the thing. The model sees the first two. Only your code ever touches the third.", "When the model decides a tool would help, it produces a small structured message: the tool's name and the arguments, in the shape the schema asked for. That's Module 2, Section 4 (structured output) doing its most important job.", "Write descriptions for a smart colleague who's never seen your system. 'Gets weather' is too thin. 'Current conditions for a city; use when the user asks about weather now, not forecasts' tells the model when to reach for it and when not to."], key: "The model chooses tools by reading their descriptions; a tool call is structured output.", question: "What does the model actually receive about a tool?", options: ["The code.", "The name, description, and argument schema.", "Nothing. It figures tools out."], answers: ["Never. It sees the name, description, and schema.", "Yes.", "It can only pick what it's told about."], deeper: "Anthropic, Building effective agents — the section on tool design.", url: "https://www.anthropic.com/research/building-effective-agents" },
