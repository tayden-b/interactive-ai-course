...
const CI_ROWS = [
  ...
  { k: "wobble", v: "outputs vary, so run the suite a few times, or take the average" },
...
`}</Code>
    ...
    title: "Project: a test suite for your agent", time: "~40 min", line: "Build an eval suite for your Module 4 agent, then change something and watch it catch the regression.", figure: "YOUR RUN  #1  91%\nYOUR RUN  #2  80%  RED\n             └ failing traces · flagged step", caption: "FIGURE 5.7.1 — A regression you caught on purpose.", paragraphs: ["This is the module people skip and the one that separates a demo from a product.", "Write the cases before touching the agent. Then break it on purpose. That's the moment the suite earns its keep.", "Run ./course init, then tell your agent to start Module 5. When your golden cases are on disk as the spec and a run has graded them with more than one kind of grader, with the pass rate honestly computed, ./course check 5 grades it against your trace, not your output. Leave ./course serve running and the panel above fills with your own run."], idea: "You can now change your agent without fear, because the suite will tell you.", deeper: "Hamel Husain, Your AI product needs evals.", url: "https://hamel.dev/blog/posts/evals/", project: true, build: ["Twenty golden cases for your agent, in a file, with \"what matters\" for each.", "Three graders: exact match on structured fields, a code check on list properties, a narrow model judge for one prose field.", "Trace assertions: \"called the right tool,\" \"no more than N steps.\"", "One command that runs everything and prints a pass rate.", "A deliberate break, like a worse tool description, and the run that catches it."], end: "A number you trust, a grid that says where failures live, and the habit of changing things only when the number agrees." },
]
