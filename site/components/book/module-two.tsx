...
const lessons = [
  {
    ...
    node: <Stack>
      <Code lang="json" title="request · sent over https" mark={[3, 4, 5, 6]}>{`
{
  "model": "…",
  "messages": [
    { "role": "system", "content": "…" },
...
SYSTEM = (
  ...
  {
    title: "Show, don't tell: examples", minutes: "6", line: "A few examples of the output you want, placed in the prompt, usually beat a paragraph describing it.", caption: "FIGURE 2.3.1 — Two examples do what a paragraph of instructions can't.", figure: "TELLING: a long instruction paragraph\n\nSHOWING: instruction → input → output\n          input → output ✓\n          real input",
    node: <Compare
...
attempt 2: success · 212 input tokens · $0.003`}</Code>
    ...
    paragraphs: ["This is the first thing you build that a person could rely on. Not because the model is reliable (it isn't) but because your code is.", "Keep the scope small. Three fields. Real notes from a real meeting. When it works, feed it the messiest notes you have and watch the retry earn its keep.", "Run ./course init, then tell your agent to start Module 2. When your script validates the output and retries with numbered attempts, ./course check 2 grades it against your trace, not your output, so printing the right answer will not pass. Leave ./course serve running and the panel above fills with your own run."], key: "Reliability lives in your code around the model, not in the model.", deeper: "Thorsten Ball, How to build an agent", url: "https://ampcode.com/how-to-build-an-agent" },
]
